...
%% constants
MAX_TIMESTEP = 200; % limit timestep for a simulation
SAVE_TIMESTEP = 100; % save result to hard drive every SAVE_TIMESTEP
...
continue_matname = 'carbonateE10p87';
continue_savenumber = 88; % must be = continue_matname + 1

microporosity_chance = 0.2; % varies this number (0, 0.01, 0.1)
micro_zeta = 0.1; 
inside_limit = 0.05; 
...
%% prev variables
all_displacement = zeros(Nparticle,3,Nrun); % particle displacement for all direction
...
if continue_simulation == true
    ...
    prev_displacement = all_displacement;
    ...
    savenumber_start = 1;
    prev_displacement = zeros(Nparticle,3,Nrun);
...
for sn = savenumber_start:Nsave
    ...
    for rn = 1:Nrun
        [run_Npe(rn),particleX(:,rn),particleY(:,rn),particleZ(:,rn),...
            all_displacement(:,:,rn), displacement(:,rn),variance(:,rn),inside_solid(:,rn)] = ...
        particletracking(g,u_org*VELOCITY_FACTORS(rn),v_org*VELOCITY_FACTORS(rn),...
            ...
            prev_displacement(:,:,rn), prev_particleX(:,rn),prev_particleY(:,rn),prev_particleZ(:,rn),...
